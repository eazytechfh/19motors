...
- Na RPC, bloqueie lead e veículo com `FOR UPDATE`, valide novamente o status e autorize vendedor responsável ou gestor.
- Se qualquer etapa falhar, reverta tudo, mantenha o modal aberto e não mostre a celebração.
...
- Atualizar lead e estoque em chamadas separadas, deixando venda fechada sem veículo vendido.
- Usar um seletor nativo enorme que abre sobre a tela, em vez do combobox pesquisável com lista abaixo.
- Aceitar texto digitado sem vincular o ID real do veículo.
- Listar veículos indisponíveis ou vendidos no modal.
- Não bloquear o veículo e permitir duas vendas concorrentes do mesmo estoque.
...
- Modal exige veículo disponível, pesquisa marca/modelo/ano/placa e abre a lista para baixo com scroll.
- Venda confirmada grava `estoque_veiculo_id` e muda o veículo para `vendido` na mesma transação.
