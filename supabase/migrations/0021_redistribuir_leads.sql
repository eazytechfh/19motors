-- Redistribui os leads selecionados igualmente entre vendedores ativos da mesma empresa.
...
begin
  ...
  select min(id_empresa), count(*), count(distinct id_empresa)
    into v_empresa, v_total_leads, v_total_empresas
  ...
  select count(*) into v_total_vendedores
   ...
   where v.id_empresa::text = v_empresa::text
     and nullif(trim(v.vendedor), '') is not null
     and exists (
       select 1
         from public.profiles p
        where lower(trim(p.nome)) = lower(trim(v.vendedor))
  ...
  with selected_leads as (
    ...
    select v.vendedor,
           row_number() over (order by lower(trim(v.vendedor)), v.id) - 1 as position
      from public."VENDEDORES" v
     where v.id_empresa::text = v_empresa::text
       and nullif(trim(v.vendedor), '') is not null
       and exists (
