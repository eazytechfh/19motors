      ...
      {podeRedistribuir && (
        <div className="flex flex-wrap items-center gap-3 border border-gray-200 bg-card p-3 shadow-sm dark:border-gray-800">
          <button
            ...
            disabled={leadsFiltrados.length === 0 || redistribuindo}
            className="border border-gray-300 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
          >
          ...
          <button
            type="button"
            onClick={() => void redistribuirSelecionados()}
            disabled={leadsSelecionados.size === 0 || redistribuindo}
            className="bg-primary px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50"
          >
          ...
          {leadsSelecionados.size > 0 && (
            <button
              type="button"
              onClick={() => setLeadsSelecionados(new Set())}
              disabled={redistribuindo}
              className="text-sm text-gray-500 hover:text-foreground dark:text-gray-400"
            >
              Limpar seleção
            </button>
          )}
          {mensagemRedistribuicao && (
            <p
              role="status"
              className={`w-full text-sm ${mensagemRedistribuicao.tipo === 'sucesso' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
            >
              {mensagemRedistribuicao.texto}
            </p>
          )}
        </div>
      )}
      ...
      <div className="rounded-xl bg-card shadow-sm">
        <div className="grid grid-cols-[32px_1.5fr_1fr_1fr_1fr_1fr_0.8fr] gap-2 border-b border-gray-200 px-4 py-3 text-xs font-semibold uppercase text-gray-500 dark:border-gray-800 dark:text-gray-400">
          <input
            ...
            disabled={!podeRedistribuir || leadsFiltrados.length === 0}
            aria-label="Selecionar todos os leads filtrados"
            className="h-4 w-4 accent-primary disabled:invisible"
          />
          <span>Lead</span>
          <span>Origem</span>
          <span>Vendedor</span>
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      onChange={() => alternarLeadSelecionado(lead.id)}
                      disabled={!podeRedistribuir}
