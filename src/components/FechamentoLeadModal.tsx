  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4">
      <div role="dialog" aria-modal="true" aria-labelledby="titulo-fechamento" className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        <label className="mt-4 block text-sm font-medium text-gray-700">Veículo vendido</label>
        <div className="relative mt-1">
          <input
            ...
            className="w-full rounded-lg border border-gray-300 px-3 py-2 pr-9"
          />
          <button
            type="button"
          ...
          {listaAberta && !carregandoVeiculos && (
            <div id="lista-veiculos-venda" role="listbox" className="absolute left-0 top-full z-[90] mt-1 max-h-60 w-full overflow-y-auto rounded-lg border border-gray-200 bg-white p-1 shadow-xl">
              {filteredVehicles.length ? filteredVehicles.map((veiculo) => (
                <button
                  ...
                  className={`block w-full rounded-md px-3 py-2 text-left text-sm hover:bg-gray-100 ${veiculoId === String(veiculo.id) ? 'bg-primary/10 font-medium text-primary' : 'text-gray-800'}`}
                >
