...
export function CustomDateRangePicker({ start, end, onStartChange, onEndChange }: CustomDateRangePickerProps) {
  ...
  return (
    <div className="flex flex-wrap items-end gap-2 rounded-lg border border-gray-200 bg-gray-50 p-2 dark:border-gray-700 dark:bg-gray-800/60">
      <label className="text-xs font-medium text-gray-600 dark:text-gray-300">
        <span className="mb-1 block">Data inicial</span>
        <input type="date" value={start} max={end || undefined}
          onChange={(event) => handleStartChange(event.target.value)}
          className="rounded-md border border-gray-300 bg-white px-2 py-1.5 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300" />
      </label>
